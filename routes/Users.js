...
function SelectOrCreateTable() {
  con.query('SELECT * FROM users', function (err, result, fields) {
    if (err) {
      const sql =
        'CREATE TABLE users (id INT AUTO_INCREMENT PRIMARY KEY, name VARCHAR(255), password VARCHAR(255), pic VARCHAR(255), address VARCHAR(255) ,  email varchar(255)  NOT NULL UNIQUE ) ';
  ...
  con.query(
    ...
    function (err, result) {
      ...
      if (result.length === 0) {
        ...
        return res
          .status(201)
          .send({ message: 'this email aleady taken before' });
      }
    }
  );
});
